...
#commands
class muh_meme_commands_bruh(commands.Cog):
    """bruh all the meme commands"""

    def __init__(self, bot):
        self.bot = bot

    @commands.command(brief="plays the requested meme bruh", description="will play the meme in muh sounds bruh that exactly matches")
    ...
    @commands.command(brief="returns the integer id for your user bruh", description="returns the integer id for your user bruh, useful for discord debug n shit")
    async def getid(context):
        message = context.message
        await message.author.send(str(message.author.id))

    @commands.command(brief="search and find up to 25 memes bruh", description="shows the first 25 memes that contain substrings that exactly match bruh")
    ...
    @commands.command(brief="sends you all the memes in a massive DM, dont", description="this will tear into your DMs and send you all the available memes, which is so many memes bro. to actually send all the memes use\n##list memes\nthis is done to prevent your destruction")
...
bot.add_cog(muh_meme_commands_bruh(bot))
#end commands
